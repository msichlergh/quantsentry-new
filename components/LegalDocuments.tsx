    ...
    sections: [
        ...
        groups: [
          {
            ...
            items: [
              "Identity and contact data — name, business email, phone, company, role.",
              "Account credentials — username, hashed password, multi-factor secrets.",
              "Communication content — messages, support tickets, sales conversations.",
              "Commercial data — billing details, contracts, purchase history.",
            ],
          },
          {
            title: "Information Collected Automatically",
            items: [
              "Device and connection data — IP address, device type, browser, operating system, language.",
              "Usage data — pages viewed, features used, timestamps, referrers.",
          ...
          },
          {
            ...
            items: [
              "Identity-verification providers (KYC/KYB checks for operator clients).",
      ...
      },
      {
        ...
        items: [
          ...
          "Onboard operator clients and meet KYC/KYB obligations (legal obligation).",
          "Detect, prevent, and respond to fraud, abuse, and security incidents (legitimate interest).",
          "Send service communications and respond to enquiries (legitimate interest / consent).",
      ...
      },
      {
        ...
        items: [
          ...
          "Service providers — cloud hosting, identity verification, analytics, payments — under written data-processing agreements.",
          ...
          "Counterparties in a corporate transaction (merger, acquisition, financing), subject to confidentiality.",
        ],
      ...
      },
      {
        ...
        paragraphs: [
          "We use essential cookies to operate our websites and authentication flows, plus analytics cookies (only with your consent) to understand product usage. You can manage cookie preferences via your browser settings or our cookie banner where shown.",
        ],
      },
      {
        id: "your-rights",
        title: "Your Rights",
        paragraphs: [
          "Subject to applicable law, you may have the right to access, correct, delete, restrict, or object to processing of your personal data, and to data portability. To exercise any right, contact us at privacy@quanttechnology.com. We will respond within the timeframe required by your jurisdiction.",
        ],
      },
      {
        id: "security",
  ...
  terms: {
    ...
    sections: [
      ...
      },
      {
        ...
        paragraphs: [
          "The services may integrate with third-party services (e.g. trading venues, payment processors, identity-verification providers). Your use of those services is governed by their own terms; QTG is not responsible for third-party services beyond what is set out in our agreements.",
        ],
      },
      {
        id: "trading-risk",
        ...
        note: "See our Disclosures page for full risk warnings and regulatory information.",
      },
      {
        id: "warranties",
      ...
      },
      {
        ...
        paragraphs: [
          "These Terms apply for as long as you use the services. Either party may terminate for material breach not cured within thirty days of written notice. Provisions that by their nature should survive termination — IP, liability, indemnity, governing law — will survive.",
        ],
      },
      {
        id: "governing-law",
...
export function LegalDocuments({ document }: { document: LegalDocument }) {
  const active: LegalDocumentContent = documents[document];

  return (
    <div className="legal-document-viewer">
      <nav aria-label="Legal documents" className="legal-document-tabs">
        {(Object.keys(documents) as LegalDocument[]).map((legalDocument) => (
          <a
            aria-current={document === legalDocument ? "page" : undefined}
            className={document === legalDocument ? "is-active" : undefined}
            href={`/${legalDocument}`}
            key={legalDocument}
          >
            {documents[legalDocument].label}
          </a>
        ))}
      </nav>

      <article
        aria-labelledby={`${document}-document-title`}
        className="legal-document-panel"
      >
        <header className="legal-document-header">
          <div>
            <span className="legal-document-label">Legal · {active.label}</span>
            <h1 id={`${document}-document-title`}>{active.title}</h1>
            <p>{active.summary}</p>
          </div>
        ...
        <div className="legal-document-layout">
          ...
          <div className="legal-document-content">
            {active.sections.map((section, index) => (
              <section id={`${document}-${section.id}`} key={section.id}>
